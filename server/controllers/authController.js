...
export const logout=async(req,res)=>{
    try{
        ...
        return res.json({success:true,messgae:'Logged Out'});
    }catch(error){
...
export const verifyEmail = async (req,res)=>{
    ...
    try{
        ...
        if(user.verifyOtp===''||user.verifyOtp!==otp){
            return res.json({success:false,messgae:'Invalid Otp'});
        }
        ...
        return res.json({success:true,messgae:'Email verified successfully'});
...
export const resetPassword=async(req,res)=>{
    ...
    try{
        const user=await userModel.findOne({email});
        if(!user){
            return res.json({success:false,messgae:'Invalid User'});
        }
        if(!user.resetOtp===''||user.resteOtp!==otp){
            return res.json({success:false,messgae:'Invalid Otp'});
        }
